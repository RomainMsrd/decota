...
multi = [['Real_World_OH', 'Clipart_OH'],['Real_World_OH', 'Product_OH'],['Real_World_OH', 'Art_OH'],['Product_OH', 'Real_World_OH'],['Product_OH', 'Clipart_OH'],['Product_OH', 'Art_OH'],['Art_OH', 'Product_OH'],['Art_OH', 'Clipart_OH'],['Art_OH', 'Real_World_OH'],['Clipart_OH', 'Real_World_OH'],['Clipart_OH', 'Art_OH'],['Clipart_OH', 'Product_OH']]
...
if args.net == 'resnet34':
    ...
    net.classifier[6] = nn.Linear(4096,len(class_list))
    #print(net)
    twin = models.vgg16(pretrained=True)
    twin.classifier[6] = nn.Linear(4096,len(class_list))
...
if args.net == 'resnet34':
    ...
    params = []
    for value in net.features.parameters():
        if value.requires_grad:
            params += [{'params': [value], 'lr': args.multi,
                        'weight_decay': 0.0005}]
    params_F1 = []
    for value in net.classifier.parameters():
        if value.requires_grad:
            params_F1 += [{'params': [value], 'lr': args.multi,
                           'weight_decay': 0.0005}]

    params_2 = []
    for value in twin.features.parameters():
        if value.requires_grad:
            params_2 += [{'params': [value], 'lr': args.multi,
                        'weight_decay': 0.0005}]

    params_F2 = []
    for value in twin.classifier.parameters():
        if value.requires_grad:
            params_F2 += [{'params': [value], 'lr': args.multi,
                           'weight_decay': 0.0005}]
...
def train():
    ...
    for step in range(args.start, all_step):
        ...
        #inversing u_1 and u_2 to disable co-training
        u_2_mask = u_1_pred[0] >= args.th
        u_1_mask = u_2_pred[0] >= args.th
        ...
        if step % args.save_interval == 0:
            ...
            acc_test_net, acc_test_twin, acc_test = test_ensemble(target_loader_test, fig3 = True)
...
def test_ensemble(loader, fig3 = False):
    net.eval()
    twin.eval()
    correct = 0
    correct_test_1 = 0
